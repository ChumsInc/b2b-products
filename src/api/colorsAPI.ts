...
export async function postColor(_color:ProductColor):Promise<{list:ProductColor[], color:ProductColor }> {
    try {
        const url = '/node-b2b/products/colors';
        const body = JSON.stringify(_color);
        const {colors, color} = await fetchJSON<{colors:ProductColor[], color: ProductColor}>(url, {method: 'POST', body});
